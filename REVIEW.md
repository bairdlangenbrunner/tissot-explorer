...
*Updated 2026-04-02 against current working tree*
...
- `src/App.tsx:80-86` and `src/App.tsx:101-107` — The size scaling toggle is a `<button>` with `aria-label="Toggle size scaling"` but no `role="switch"` or `aria-checked` attribute. Assistive technology can't communicate the toggle's on/off state.
...
- **Fix:** Add an `id` to a heading inside the modal and reference it with `aria-labelledby`.

**Major — Modal close button uses sticky float pattern**
- `src/components/AboutModal.tsx:54` — The close button has `aria-label="Close"` (good), but the `position: sticky; float: right` CSS pattern can cause focus order confusion since it's the first focusable element but visually positioned top-right.
...
- `src/components/ProjectionPicker.tsx:49` — Projection buttons are `<button>` elements inside plain divs. There's no `role="group"` or `aria-label` on the groups, so a screen reader user doesn't know they're navigating "Conformal" vs "Equal-area" projections.
...
- CSS transitions on toggle knob, dropdown arrow, and hint opacity don't respect `prefers-reduced-motion`.
...
- `src/App.tsx:66-71` — These are fine for sighted users but terse labels like "random" don't communicate purpose. Minor since the visible text itself is readable.

~~**Minor — Slider thumb is 14px (below 44px touch target)**~~ **FIXED**
- The slider thumb is now 24×24px, up from the original 14px. Still below the 44px WCAG recommendation for touch targets, but a reasonable improvement.
...
- The CSS defines a `.hint` class but it is not rendered anywhere in the current JSX. There is no visible onboarding guidance — users land on a random projection with no explanation of what to do.
...
- `max-height: calc(100dvh - 120px)` could still be tight with many projections. The dropdown scrolls, which is good, but there's no visual scroll indicator.
...
- `src/components/MapCanvas.tsx:41` — `renderMap()` is called inside the world-data fetch `useEffect` (lines 32-43), but the effect has an empty dependency array `[]` while `renderMap` is a `useCallback` that changes when `projectionIndex`, `centralLon`, or `centralLat` change. The initial call works because a separate `useEffect` at line 152-154 also calls `renderMap` on `[renderMap]`. However, the stale closure in the fetch effect means if the fetch resolves after a projection change, it will call an outdated `renderMap`.
...
- `src/components/MapCanvas.tsx:54` — `window.matchMedia('(max-width: 640px)')` runs inside `renderMap` on every call. Use a cached result or a `useMediaQuery` hook.

**Minor — No error handling for world data fetch**
- `src/components/MapCanvas.tsx:33-42` — If the jsDelivr CDN is down, the app silently shows an empty map with just a graticule. No retry, no fallback, no error state.
- **Fix:** Add a `.catch()` handler and a fallback message.

**Minor — Inline SVG icons could be extracted**
- `src/App.tsx:61-63` (undo icon) and `src/App.tsx:117-131` (sun/moon theme icons) — inline SVG in JSX. Not a major issue, but extracting to small components would improve readability.

**Minor — CSS uses hardcoded colors instead of variables**
- `.controls button.active` uses `#d04e2e` and `#fff` instead of `var(--accent)` and a white variable, so the active button color won't adapt if themes are extended.
...
- The info panel at `bottom: 20px; left: 28px` is `pointer-events: none` (good) but can obscure map features in the lower-left. No background means text can be hard to read against certain land/ocean colors.
...
- Fixed `bottom: 20px; right: 20px` can overlap with browser UI or scrollbar in landscape mode.
...
| Issue | Resolution |
|-------|-----------|
| Group tooltips inaccessible on desktop | Added `role="tooltip"`, `aria-describedby`, keyboard focus + Escape dismiss |
| Slider thumb too small | Increased to 24×24px |
| Central longitude resets on projection change | Longitude now preserved across changes and undo |
